<template>
  <div v-if="visible" :style="styleObject" class="context-menu" @click.stop>
    <ul class="menu-list">
      ...
      <li v-if="this.$store.state.selectedNodes.length === 2" @click="createLink">{{ $t('knowledgeGraph.createlink') }}
      </li>
...
export default {
  ...
  unmounted() { // Correct lifecycle hook name is 'unmounted' in Vue 3
