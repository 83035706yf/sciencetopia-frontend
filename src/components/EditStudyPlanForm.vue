<template>
  <v-form @submit.prevent="saveStudyPlan">
    ...
    <!-- Introduction Input -->
    <v-textarea v-model="localStudyPlan.introduction.description" :label="$t('studyplan.introduction')"
      :rules="[rules.required]" :auto-grow="true" rows="1" max-rows="5" variant="outlined"
        ...
        <div class="resource-section">
          ...
          <div v-for="(resource, rIndex) in lesson.resources" :key="rIndex" class="resource-content">
            <div class="resource-header">
              ...
              <v-btn size="small" icon variant="text" @click="removeResource('prerequisite', index, rIndex)">➖️</v-btn>
            </div>
            <v-text-field class="lesson-description" v-model="resource.link" :label="$t('studyplan.resourcelink')"
    ...
    <div class="scrollable-section" v-if="localStudyPlan.advancedTopics.length">
      <div v-for="(lesson, index) in localStudyPlan.advancedTopics" :key="index" class="lesson-section">
        ...
        <div class="resource-section">
          <div class="section-header">
            <h5>{{ $t('studyplan.resouces') }}</h5>
            <h5 v-if="lesson.resources[0] == null">{{ $t('studyplan.empty') }}</h5>
